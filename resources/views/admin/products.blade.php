        ...
        <form id="productCreateForm" action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <label class="flex items-center gap-2 mb-4">
                <input type="checkbox" id="useVariants" name="use_variants" value="1"
                       class="w-4 h-4" {{ old('use_variants') ? 'checked' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="file" name="gambar"
                        class="w-full border rounded-lg px-3 py-2 @error('gambar') border-red-500 @enderror" required>
                    @error('gambar')
                        <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($products as $index => $product)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4">
                            @if($product->gambar)
                                <img src="{{ asset('storage/' . $product->gambar) }}" width="60" class="rounded-md">
                            @else
                                -
                            @endif
                        </td>
